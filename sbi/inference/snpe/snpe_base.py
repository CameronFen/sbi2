...
class PosteriorEstimator(NeuralInference, ABC):
    ...
    def train(
        ...
        if calibration_kernel is None:
            calibration_kernel = lambda x: ones([training_batch_size], device=self._device)
        ...
        for indx,ele in enumerate(x):
            ...
            datasetvec.append(Data(x=ele[0], edge_index=ele1, y = torch.cat((theta[indx].reshape(1,-1),prior_masks[indx].reshape(1,-1)), axis = 1)))
